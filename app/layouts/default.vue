<template>
  <div :class="['bg-gradient-to-br transition-all duration-500 relative', currentGradientClass]">
        ...
        <div class="header-inner">
          ...
          <div class="theme-selector">
            <button v-for="themeOption in themes" :key="themeOption.name" @click="setTheme(themeOption.name)"
              :class="[
                'w-10 h-8 rounded-full border-2 transition-all transform hover:scale-110',
...
.header-inner h1 {
  @apply text-4xl font-bold text-white mb-2 animate-fade-in;
}

.header-inner p {
  @apply text-gray-300 text-sm animate-fade-in;
}

.theme-selector {
  @apply flex gap-2 items-center p-1 animate-fade-in;
}
