...
export function useWordValidation() {
  const extractWordsFromBoard = (
    ...
    console.log(`[WORD-EXTRACTION] Temp letters on same ${allSameRow ? 'row' : 'column'}`);
    ...
    if (allSameRow) {
      ...
      console.log(`[WORD-EXTRACTION] Checking row ${row}, cols ${minCol}-${maxCol}`);
      ...
      console.log(`[WORD-EXTRACTION] Extended sequence: cols ${startCol}-${endCol}`);
      ...
      if (!hasPermanent) {
        console.log('[WORD-EXTRACTION] ✗ No permanent letters in sequence (cannot create isolated words)');
        return [];
      }
      ...
      console.log(`[WORD-EXTRACTION] Checking column ${col}, rows ${minRow}-${maxRow}`);
      ...
      console.log(`[WORD-EXTRACTION] Extended sequence: rows ${startRow}-${endRow}`);
    ...
    console.log(`[WORD-EXTRACTION] ✓ Generated ${wordArray.length} word combinations`);
    wordArray.forEach((word, i) => {
      console.log(`[WORD-EXTRACTION]   ${i + 1}. "${word}" (${word.length} letters)`);
    });

    return wordArray;
  };

  const validateWordsOnServer = async (words: string[], usedWords: string[]) => {
