        ...
        nitroApp.router.use("/socket.io/", defineEventHandler({
            handler(event) {
                try {
                    ...
                    console.log(`[SOCKET.IO] HTTP ${event.node.req.method} ${event.node.req.url}`);
            ...
            socket.on('join-room', async (data: { roomCode: string; playerId: string; playerName: string }) => {
                try {
                    ...
                    if (!roomCode || !playerId || !playerName) {
                        console.error('[SOCKET.IO] ✗ Missing fields:', { roomCode, playerId, playerName });
                        socket.emit('error', { message: 'Missing required fields' });
                        return;
                    }

                    console.log(`[SOCKET.IO] JOIN-ROOM: ${playerName} (${playerId}) -> ${roomCode}`);
                    ...
                    console.log(`[SOCKET.IO] ✓✓ ${playerName} in ${roomCode}. Players: ${room.players.length}`);
                } catch (err) {
                    console.error('[SOCKET.IO] ✗ join-room error:', err);
                    socket.emit('error', { message: 'Failed to join', details: String(err) });
                }
            });
            ...
            socket.on('leave-room', async (data: { roomCode: string; playerId: string }) => {
                try {
                    ...
                    if (room) {
                        ...
                        console.log(`[SOCKET.IO] ✓ Player left ${roomCode}. Players: ${room.players.length}`);
                    }
                } catch (err) {
            ...
            socket.on('update-game-state', async (data: { roomCode: string; gameState: any }) => {
                try {
                    ...
                    if (!roomCode || !gameState) {
                        console.error('[SOCKET.IO] ✗ Missing fields in update-game-state');
                        return;
                    }
